...
check_installer() {
    ...
    if [[ "$size" -lt 50000 ]]; then
        echo -e "${YELLOW}[*] 文件太小 ($size bytes)，可能是错误页面${NC}"
        return 1
    fi
    # 检查是否是 HTML
    if head -c 200 "$file" | grep -qi '<html\|<!doctype\|<!DOCTYPE'; then
        echo -e "${YELLOW}[*] 检测到 HTML 内容，可能是重定向页面${NC}"
        return 1
    fi
    # 检查是否有 shell 脚本特征
    if ! head -c 50 "$file" | grep -q '#!'; then
        echo -e "${YELLOW}[*] 不是有效的 shell 脚本${NC}"
        return 1
    fi
    return 0
}
...
download_acme() {
    ...
    if wget -q --timeout=30 -O "$ACME_INSTALLER" https://get.acme.sh 2>&1; then
        if check_installer "$ACME_INSTALLER"; then
            echo -e "${GREEN}[*] get.acme.sh 下载成功${NC}"
            return 0
        fi
    fi
    ...
    if wget -q --timeout=30 -O "$ACME_INSTALLER" https://raw.githubusercontent.com/acmesh-official/acme.sh/master/acme.sh 2>&1; then
        if check_installer "$ACME_INSTALLER"; then
    ...
    # 源3: 备用镜像
    echo -e "${YELLOW}[*] 尝试从备用镜像下载...${NC}"
    if wget -q --timeout=30 -O "$ACME_INSTALLER" https://cdn.jsdelivr.net/gh/acmesh-official/acme.sh@master/acme.sh 2>&1; then
        if check_installer "$ACME_INSTALLER"; then
...
if ! download_acme; then
    echo -e "${RED}[错误] acme.sh 从所有源下载均失败${NC}"
    echo -e "${YELLOW}[提示] 请检查网络连接，或手动安装 acme.sh:${NC}"
    echo -e "  wget -O - https://get.acme.sh | sh"
    rm -rf "$ACME_DIR"
    exit 1
fi
